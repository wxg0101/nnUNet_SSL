...
class nnUNetTrainerV2_SSL(nnUNetTrainer):
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            ################# END ###################
            ################# LOAD DATA#################
            self.dataset_directory_2 = '/media/oem/sda21/wxg/codebase/dataset/nnUNet_preprocessed/Task521_LUNGSeg'
            ...
            if training:
                ...
                self.tr_gen_2, self.val_gen_2 = get_moreDA_augmentation(
                    self.dl_tr_2, self.dl_val_2,
                    self.data_aug_params[
                        'patch_size_for_spatialtransform'],
                    self.data_aug_params,
                    deep_supervision_scales=self.deep_supervision_scales,
    ...
    def get_basic_generators_2(self):
        self.load_dataset_2()
        self.do_split_2()

        if self.threeD:
            dl_tr = DataLoader3D(self.dataset_tr_2, self.basic_generator_patch_size, self.patch_size, self.batch_size,
                                 False, oversample_foreground_percent=self.oversample_foreground_percent,
                                 pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
            dl_val = DataLoader3D(self.dataset_val_2, self.patch_size, self.patch_size, self.batch_size, False,
    ...
    def initialize_network(self):
        ...
        self.ema_model = Generic_UNet(self.num_input_channels, self.base_num_features, self.num_classes,
                                    len(self.net_num_pool_op_kernel_sizes),
                                    self.conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,
                                    dropout_op_kwargs,
                                    net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
        ...
        self.ema_model.inference_apply_nonlin = softmax_helper
    ...
    def do_split_2(self):
        tr_keys_2 = val_keys_2 =list(self.dataset_2.keys())
        tr_keys_2.sort()
        val_keys_2.sort()
        self.dataset_tr_2 = OrderedDict()
        for i in tr_keys_2:
            self.dataset_tr_2[i] = self.dataset_2[i]
        self.dataset_val_2 = OrderedDict()
        for i in val_keys_2:
            self.dataset_val_2[i] = self.dataset_2[i]
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                ...
                for _ in range(self.num_batches_per_epoch):
                    ...
                    l = self.run_iteration(self.tr_gen,self.tr_gen_2, True,iter_num=iter_num)
                    iter_num=iter_num+1
                    train_losses_epoch.append(l)
